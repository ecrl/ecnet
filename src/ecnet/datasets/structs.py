...
from .utils import _qspr_from_alvadesc, _qspr_from_alvadesc_smifile, _qspr_from_padel
...
class QSPRDataset(Dataset):
    ...
    def set_index(self, index: List[int]):
        ...
        index_t = torch.as_tensor(index, dtype=torch.long)
        self.smiles = [self.smiles[i] for i in index]
        self.target_vals = self.target_vals[index_t]
        self.desc_vals = self.desc_vals[index_t]
...
class QSPRDatasetFromFile(QSPRDataset):
    def __init__(
        ...
        self.target_vals = torch.as_tensor(target_vals).type(torch.float32)
        if backend == "padel":
            self.desc_vals, self.desc_names = self.smi_to_qspr(self.smiles, backend)
            self.desc_vals = torch.as_tensor(self.desc_vals).type(torch.float32)
        elif backend == "alvadesc":
            self.desc_vals, self.desc_names = _qspr_from_alvadesc_smifile(smiles_fn)
            self.desc_vals = torch.as_tensor(self.desc_vals).type(torch.float32)
...
class PCADataset(QSPRDataset):
    def __init__(
        ...
        target_vals: Iterable[Iterable[float]],
        backend: str = "padel",
        existing_pca_dataset: "PCADataset" = None,
    ):
        """
        ...
        self.desc_vals = torch.as_tensor(self.pca.transform(desc_vals)).type(
            torch.float32
        )
